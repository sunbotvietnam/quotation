...
const tests = [
  ...
  ["standalone outputs use clean A4 window", () => { assert.match(documentOutput, /window\.open/); assert.match(documentOutput, /@page\{size:A4 portrait;margin:12mm 14mm 13mm\}/); assert.match(documentOutput, /sanitizeClone/); }],
  ["three independent customer documents exist", () => { assert.match(documentOutput, /Báo giá/); assert.match(documentOutput, /Đề xuất đầy đủ/); assert.match(documentOutput, /Thuyết minh/); assert.match(documentOutput, /mode === "quote"/); assert.match(documentOutput, /mode === "narrative"/); }],
