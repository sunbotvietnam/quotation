// Authentication UI for Sunbot quotation app.
(function () {
  ...
  regionOf = function () { return ""; };

  creatorOptions = function () {
    if (state.role === "ADMIN") {
      return '<option value="">-- Chọn người lập báo giá --</option>' + MANAGERS.map(name => `<option value="${esc(name)}" ${state.createdBy === name ? "selected" : ""}>${esc(name)}</option>`).join("");
    }
    const manager = state.user;
    if (!manager?.display_name) return '<option value="">-- Tài khoản chưa gắn người lập báo giá --</option>';
  ...
  loginView = function (msg = "") {
    app.innerHTML = `<div class="wrap"><section class="panel login shared-login">
      ...
      <p class="help">Dùng ID và mật khẩu nội bộ được cấp.</p>
    </section></div>`;
  ...
  logout = function () { baseLogout(); sessionStorage.removeItem(LOGIN_KEY); state.loginId = ""; };
