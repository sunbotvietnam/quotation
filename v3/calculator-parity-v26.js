...
(function(){
  ...
  const MANAGED_IDS=new Set(['PROGRAM_FEE_FORMULA','DEPLOYMENT_SITE_QA','EQUIPMENT_CAPITAL_RECOVERY','TRAIN_1','TRAIN_2','TRAIN1_EXTRA10','TRAIN2_EXTRA10','RETRAIN_1','RETRAIN_2','RETRAIN1_EXTRA10','RETRAIN2_EXTRA10','CERT_1','CERT_2',...EQUIPMENT_IDS]);
  ...
  function updateEquipmentNarrative(b,x){
    const spec=EQUIPMENT_IDS.map(id=>`${catalogItem(id)?.name||id}: ${Math.max(0,num(ensureEquipment(b)[id]))}`).join('; '),reason=String(b.equipment_adjustment_reason||'').trim();
    const block=`[[EQUIPMENT_CONFIG_BEGIN]]\nCấu hình thiết bị: ${spec}. Giá trị cấu hình: ${money(x.capital)}.${reason?` Lý do điều chỉnh dưới chuẩn: ${reason}.`:''}\n[[EQUIPMENT_CONFIG_END]]`;
    const current=String(b.configuration_description||''),rx=/\[\[EQUIPMENT_CONFIG_BEGIN\]\][\s\S]*?\[\[EQUIPMENT_CONFIG_END\]\]/;
    b.configuration_description=rx.test(current)?current.replace(rx,block):(current?current+'\n\n':'')+block;
  }
  applyTemplate=function(b){
    if(!b||b.kind!=='SOLUTION')return;
    const extras=(b.lines||[]).filter(l=>!MANAGED_IDS.has(String(l.item_id||''))).map(l=>Object.assign({},l));
  ...
  publishBuilder=async function(kind){
    if(kind!=='SOLUTION')return oldPublish(kind);syncBuilderFromForm();const b=state.builder,x=calc(b);
    if(!String(b.customer_name||'').trim())return setNotice('Hãy nhập tên trường/khách hàng.','error');if(x.blocked)return setNotice(x.blocked,'error');if(x.under.length&&!String(b.equipment_adjustment_reason||'').trim())return setNotice('Cấu hình thiết bị đang thấp hơn chuẩn máy tính phương án. Hãy ghi lý do kiểm kê/thiết bị tương thích hoặc phê duyệt ngoại lệ trước khi phát hành.','error');if(num(b.discount_pct)>7&&!String(b.ceo_approval_note||'').trim())return setNotice('Giảm phí chương trình trên 7% cần ghi phê duyệt ngoại lệ CEO trước khi phát hành.','error');
    applyTemplate(b);
    try{const created=await withBusy('Đang lập báo giá theo phương án đã khóa…',()=>bridge('quotationShared','createCommercialQuote',payloadFromBuilder(b)));await withBusy('Đang phát hành và tạo tài liệu Drive…',()=>bridge('quotationShared','approveQuote',{quote_id:created.quote_id}));await refreshData(false);state.tab='quotes';state.builder=null;setNotice(`Đã phát hành ${codeText(created.quote_id)} theo logic máy tính phương án.`);setTimeout(()=>openDocumentLinks(created.quote_id),50)}catch(er){setNotice(errText(er),'error')}
  };
})();
